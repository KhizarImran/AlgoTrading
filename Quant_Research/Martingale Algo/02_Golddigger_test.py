...
# def close_positions_by_type(symbol, position_type):
#     positions = mt5.positions_get(symbol=symbol)
#     for position in positions:
#         if (position_type == 'buy' and position.type == mt5.ORDER_TYPE_BUY) or \
#            (position_type == 'sell' and position.type == mt5.ORDER_TYPE_SELL):
#             if position.type == mt5.ORDER_TYPE_BUY:
#                 order_type = mt5.ORDER_TYPE_SELL
#                 price = mt5.symbol_info_tick(symbol).bid
#             else:
...
def volume(symbol, current_volume):
    ...
    last_pnl_value = last_pnl(symbol)
    if last_pnl_value < 0:
        calculated_volume = calculate_volume(False, current_volume)
        print(f"Last closed trade was a loss. Doubling volume to {calculated_volume}")
    elif last_pnl_value >= 0:
        calculated_volume = calculate_volume(True, current_volume)
        print(f"Last closed trade was a win. Resetting volume to {calculated_volume}")
    else:
        calculated_volume = current_volume
        print(f"No previous trade found. Keeping volume at {calculated_volume}")
...
def last_pnl(symbol):
    try:
        ...
        history_deals = mt5.history_deals_get(start_date, end_date, symbol=symbol)
        if history_deals is None:
            print("No history deals found")
            return 0
        
        data = pd.DataFrame(list(history_deals), columns=history_deals[0]._asdict().keys())
        profit_or_loss = data[data['profit'] != 0]
...
def main():
    ...
    # Initialize the TradingMetaTraderForex class
    trader = TradingMetaTraderForex(login=5027102560, password='@c2qDxIf', server='MetaQuotes-Demo', symbol=symbol)
    
    if not trader.initialise:
        print("Failed to initialize the Metatrader 5 library.")
        return
    ...
    while True:
        try:
            ...
            if len(trader.account_total_number_of_positions_info) != 0:
                positions = trader.account_total_number_of_positions_info
            else:
                positions = []
            ...
            print(datetime.now(),
                  '| Balance:', trader.account_info_balance,
                  '| Equity:', trader.account_info_equity,
                  '| Profit:', trader.account_info_profit)
            ...
            if last_ceiling_price is not None and last_floor_price is not None:
                if current_price > last_ceiling_price and len(positions) == 0:
                    if len(buy_positions) < max_positions:
                        ...
                        trader.make_request(symbol, current_volume, 'buy', sl=current_price-0.75, tp=current_price+0.95)
                ...
                if current_price < last_floor_price and len(positions) == 0:
                    if len(sell_positions) < max_positions:
                        ...
                        current_volume = volume(symbol, current_volume)
                        trader.make_request(symbol, current_volume, 'sell', sl=current_price+0.75, tp=current_price-0.95)
