...
static input double Entry_Amount =     0.10; // Entry lots
       input int    Stop_Loss    =     3000; // Stop Loss   (pips)
       input int    Take_Profit  =     2500; // Take Profit (pips)
...
static input string ___1______   = "------"; // --- Williams' Percent Range ---
       ...
       input double Ind1Param1   =    -70.0; // Level
static input string _Settings___ = "------"; // --- Expert Settings ---
static input int    Magic_Number = 84024123; // Magic Number
...
void InitIndicators()
  {
   ...
   // Williams' Percent Range (50), Level: -70.0
...
int GetEntrySignal()
  {
   // Directional Indicators (9)
   double ind0buffer0[]; CopyBuffer(indHandlers[0][0][0], 1, 1, 3, ind0buffer0);
   double ind0buffer1[]; CopyBuffer(indHandlers[0][0][0], 2, 1, 3, ind0buffer1);
...
//+------------------------------------------------------------------+
/*STRATEGY MARKET Premium Data; XAUUSD; M5 */
/*STRATEGY CODE {"properties":{"entryLots":0.1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":3000,"takeProfit":2500,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Directional Indicators","listIndexes":[0,0,0,0,0],"numValues":[9,0,0,0,0,0]}],"closeFilters":[{"name":"Williams' Percent Range","listIndexes":[4,0,0,0,0],"numValues":[50,-70,0,0,0,0]}]} */
